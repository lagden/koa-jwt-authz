 ...
 * @api private
 */
function _error(ctx, expectedScopes, errorMessage = 'Insufficient scope') {
...
function jwtAuthz(expectedScopes, options = {}) {
	...
	return async (ctx, next) => {
		...
		if (typeof user[scopeKey] === 'string') {
			userScopes = user[scopeKey].split(' ')
		} else if (Array.isArray(user[scopeKey])) {
			userScopes = user[scopeKey]
		} else {
			_error(ctx, expectedScopes)
		}

		const methods = ['some', 'every']
		const position = checkAllScopes ? 1 : 0
		const allowed = expectedScopes[methods[position]](scope => userScopes.includes(scope))
